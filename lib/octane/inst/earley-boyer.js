var EarleyBoyer = new BenchmarkSuite('EarleyBoyer', [666463], [
    new Benchmark('Earley', true, false, 2500, function () {
...
sc_Pair.prototype.toString = function () {
    return $disclosure96.sc_toDisplayString(this);
};
...
sc_Pair.prototype.sc_toDisplayString = function () {
    return this.sc_toWriteOrDisplayString($disclosure96.sc_toDisplayString);
};
sc_Pair.prototype.sc_toWriteString = function () {
    return this.sc_toWriteOrDisplayString($disclosure96.sc_toWriteString);
};
...
function sc_makePromise(proc) {
    var $disclosure345 = $disclosure.create(arguments);
    this.$disclosure_index = [
        345,
        $disclosure345.$ref_index
    ];
    $disclosure345.$scopes.$disclosure345 = $disclosure345.$ref_index;
    $disclosure345.$scopes.$disclosure96 = $disclosure96.$ref_index;
    $disclosure345.proc = proc;
    $disclosure345.isResultReady = false;
    $disclosure345.result = undefined;
    return $disclosure345.$ret_func = function () {
        if (!$disclosure345.isResultReady) {
            var tmp = $disclosure345.proc();
            if (!$disclosure345.isResultReady) {
                $disclosure345.isResultReady = true;
                $disclosure345.result = tmp;
            }
        }
        return $disclosure345.result;
    };
}
...
sc_Tokenizer.prototype.nextToken = function () {
    var $disclosure473 = $disclosure.create(arguments);
    this.$disclosure_index = [
        473,
        $disclosure473.$ref_index
    ];
    $disclosure473.$scopes.$disclosure473 = $disclosure473.$ref_index;
    $disclosure473.port = this.port;
    $disclosure473.isNumberChar = function isNumberChar(c) {
        return c >= '0' && c <= '9';
    };
    ;
    $disclosure473.isIdOrNumberChar = function isIdOrNumberChar(c) {
        return SC_ID_CLASS.indexOf(c) != -1 || c >= '0' && c <= '9';
    };
    $disclosure473.isWhitespace = function isWhitespace(c) {
        return c === ' ' || c === '\r' || c === '\n' || c === '\t' || c === '\f';
    };
    ;
    $disclosure473.isWhitespaceOrEOF = function isWhitespaceOrEOF(c) {
        return $disclosure473.isWhitespace(c) || c === SC_EOF_OBJECT;
    };
    ;
    $disclosure473.readString = function readString() {
        res = '';
        while (true) {
            var c = $disclosure473.port.readChar();
            switch (c) {
            case '"':
                return new sc_Token(11, res);
            case '\\':
                var tmp = $disclosure473.port.readChar();
                switch (tmp) {
                case '0':
                    res += '\0';
                    break;
                ...
                case 'x':
                    var nb = 0;
                    while (true) {
                        var hexC = $disclosure473.port.peekChar();
                        if (hexC >= '0' && hexC <= '9') {
                            $disclosure473.port.readChar();
    ...
    ;
    $disclosure473.readIdOrNumber = function readIdOrNumber(firstChar) {
        var res = firstChar;
        while ($disclosure473.isIdOrNumberChar($disclosure473.port.peekChar()))
            res += $disclosure473.port.readChar();
        if (isNaN(res))
    ...
    ;
    $disclosure473.skipWhitespaceAndComments = function skipWhitespaceAndComments() {
        var done = false;
        while (!done) {
            done = true;
            while ($disclosure473.isWhitespace($disclosure473.port.peekChar()))
                $disclosure473.port.readChar();
            if ($disclosure473.port.peekChar() === ';') {
                $disclosure473.port.readChar();
                done = false;
                while (true) {
                    $disclosure473.curChar = $disclosure473.port.readChar();
                    if ($disclosure473.curChar === SC_EOF_OBJECT || $disclosure473.curChar === '\n')
                        break;
                }
            }
        }
    ...
    ;
    $disclosure473.readDot = function readDot() {
        if ($disclosure473.isWhitespace($disclosure473.port.peekChar()))
            return new sc_Token(10);
        else
            return $disclosure473.readIdOrNumber('.');
    };
    ;
    $disclosure473.readSharp = function readSharp() {
        var c = $disclosure473.port.readChar();
        if ($disclosure473.isWhitespace(c))
            return new sc_Token(13, 'bad #-pattern0.');
        if ($disclosure473.isNumberChar(c)) {
            var nb = c - 0;
            while ($disclosure473.isNumberChar($disclosure473.port.peekChar()))
                nb = nb * 10 + ($disclosure473.port.readChar() - 0);
            switch ($disclosure473.port.readChar()) {
            case '#':
        ...
        if (c === '\\') {
            var tmp = '';
            while (!$disclosure473.isWhitespaceOrEOF($disclosure473.port.peekChar()))
                tmp += $disclosure473.port.readChar();
            switch (tmp.length) {
            case 0:
                if (sc_isEOFObject($disclosure473.port.peekChar))
                    return new sc_Token(13, 'bad #-pattern2.');
                else
                    return new sc_Token(20, $disclosure473.port.readChar());
            case 1:
        ...
        while (true) {
            c = $disclosure473.port.peekChar();
            if (($disclosure473.isWhitespaceOrEOF(c) || c === ')') && needing == '')
                return res;
            else if ($disclosure473.isWhitespace(c) || needing == '')
    ...
    ;
    $disclosure473.skipWhitespaceAndComments();
    $disclosure473.curChar = $disclosure473.port.readChar();
    if ($disclosure473.curChar === SC_EOF_OBJECT)
        return $disclosure473.$ret_func = new sc_Token(0, $disclosure473.curChar);
    switch ($disclosure473.curChar) {
    case ' ':
    case '\n':
    case '\t':
    ...
    case '#':
        return $disclosure473.$ret_func = $disclosure473.readSharp();
    case '.':
        return $disclosure473.$ret_func = $disclosure473.readDot();
    case '"':
        return $disclosure473.$ret_func = $disclosure473.readString();
    default:
        if ($disclosure473.isIdOrNumberChar($disclosure473.curChar))
            return $disclosure473.$ret_func = $disclosure473.readIdOrNumber($disclosure473.curChar);
        throw 'unexpected character: ' + $disclosure473.curChar;
    }
};
...
var sc_gensym = function () {
    var $disclosure627 = $disclosure.create(arguments);
    this.$disclosure_index = [
        627,
        $disclosure627.$ref_index
    ];
    $disclosure627.$scopes.$disclosure627 = $disclosure627.$ref_index;
    $disclosure627.counter = 1000;
    return $disclosure627.$ret_func = function (sym) {
        $disclosure627.counter++;
        if (!sym)
            sym = sc_SYMBOL_PREFIX;
        return sym + 's' + $disclosure627.counter + '~' + '^sC-GeNsYm ';
    };
}();
...
var const_earley;
{
    ...
    BgL_makezd2parserzd2 = function (grammar, lexer) {
        var $disclosure698 = $disclosure.create(arguments);
        this.$disclosure_index = [
            698,
            $disclosure698.$ref_index
        ];
        $disclosure698.grammar = grammar;
        $disclosure698.lexer = lexer;
        var i;
        var parser_descr;
        var def_loop;
        var nb_nts;
        ...
        var ind;
        {
            ...
            return $disclosure698.$ret_func = function (input) {
                var optrOpnd;
                var sc_optrOpnd_20;
                var sc_optrOpnd_21;
                var sc_optrOpnd_22;

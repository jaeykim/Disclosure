...
// 		var stringified = $disclosure.serialize();
...
var $disclosure = (function() {
	...
	return {
		...
		serialize: function() {
			...
			function iterativeStringify(obj, name, is_disclosure){
				...
				var $scopes;
				var $disclosure_index = [];
				if(is_disclosure)var disclosure_start = true;
				if(is_disclosure)var disclosure_object = obj;
				while(workList.length){
					...
					else if(currentObj == $disclosure){
						stringified += currentName + " = $disclosure;\n";
						continue;
					}
					else if(currentObj == $disclosure0){
						stringified += currentName + " = $disclosure0;\n";
						continue;
					}
					if(currentObj && currentObj.$disclosure_index){
						$disclosure_index[currentObj.$disclosure_index[0]] = currentObj.$disclosure_index[1];
						var ref_table_index = currentObj.$disclosure_index[1];
						for(var scope in ref_table[ref_table_index].$scopes){
							var scope_num = parseInt(scope.slice(4));
							$disclosure_index[scope_num] = ref_table[ref_table_index].$scopes[scope];
						}
					}
					if(currentObj && currentObj.$scopes){
						...
						for(var scope in currentObj.$scopes){
							var scope_num = parseInt(scope.slice(4));
							$disclosure_index[scope_num] = currentObj.$scopes[scope];
						}
					}
					if(!is_disclosure && (currentObj instanceof Function || currentObj instanceof Array || currentObj instanceof Object))var disclosure_index = lookup(currentObj);
					...
					if(currentObj && currentObj.isRestored && (!(currentObj.__proto__) || currentObj.isRestored != currentObj.__proto__.isRestored)){
					...
					else if(disclosure_index > -1){
						stringified += currentName + " = $disclosure.get_ref(" + disclosure_index + ");\n";
					}
					else if(currentObj instanceof Function){
						var temp_stringified = currentName + " = " + currentObj.disclosure_toString() + ";\n";
						for(var i in $disclosure_index){
							if(!temp_stringified.includes("$disclosure" + i + " = $disclosure.create"))temp_stringified = temp_stringified.substr(0, temp_stringified.indexOf(" function ")) + replaceAll(temp_stringified.substr(temp_stringified.indexOf(" function ")), "$disclosure" + i, "($disclosure.get_ref(" + $disclosure_index[i] + "))");
						}
						for(var i in $scopes){
							if(!temp_stringified.includes(i + " = $disclosure.create")){
								temp_stringified = temp_stringified.substr(0, temp_stringified.indexOf(" function ")) + replaceAll(temp_stringified.substr(temp_stringified.indexOf(" function ")), i, "($disclosure.get_ref(" + $scopes[i] + "))");
							}
						}
						if(is_disclosure){
							var event_index = lookupEventTable(currentObj);
							if(event_index > -1){
								temp_stringified = temp_stringified.substr(0, temp_stringified.indexOf(" function ")) + temp_stringified.substr(temp_stringified.indexOf(" function ")).replace(/\$disclosure[0-9]+/g, '($disclosure.get_ref(' + event_index + '))');
							}
						}
						...
						if(currentObj.disclosure_toString().match(reg)[0] != object_string){
							var isCSS = (currentObj.disclosure_toString().indexOf("CSSStyleDeclaration") > -1);
							if(isCSS){
							...
							if(currentObj){
								currentObj.isRestored = currentName.replace(/(\$disclosure)([0-9]+)/, function(match, p1, p2, offset, string){
									return "($disclosure.get_ref(" + p2 + "))";
								});
							}
							continue;
						}
						/*
						if(currentObj.disclosure_toString().indexOf("HTML") > -1){
							stringified += currentName + " = " + getHTMLElementPath(currentObj, currentName) + ";\n";
							continue;
						}
						else if(currentObj.disclosure_toString().indexOf("Canvas") > -1){
							stringified += currentName + " = " + getHTMLElementPath(currentObj, currentName) + ";\n";
							continue;
						}
						*/
			...
			function stringify(obj, name){
				if(obj instanceof Function){
					var str = obj.disclosure_toString() + "\n";
					// Property Traverse
					for(var i in obj){
					...
					if(obj.disclosure_toString().indexOf("HTML") > -1)return getHTMLElementPath(obj);
					var $ref = obj.$disclosure_index;
					var stringified = "{";
					for(var i in obj){
					...
					stringified += "}\n";
					if($ref){
						stringified = replaceAll(stringified, "$disclosure" + $ref[0], "($disclosure.get_ref(" + $ref[1] + "))");
					}
					return stringified;
			...
			function stringifyPrototype(obj, name){
				if(obj instanceof Function){
					...
					var stringified = "";
					if(obj.disclosure_toString().indexOf("HTML") > -1)return stringified;
					for(var i in obj){
						if(obj.hasOwnProperty(i)){
							stringified += stringifyPrototype(obj[i], name + "." + i);
						}
			...
			code += "$disclosure.restoreDOM(rawHTML);\n\n";
            ...
            for (let i = 1; i < lengthOfWeakMap(ref_table); i++) {
				var new_disclosure = "$disclosure" + i;
				code += "\tvar " + new_disclosure + " = $disclosure.create();\n";
				code += "\t" + iterativeStringify(ref_table[i], new_disclosure, true) + ";\n";
				continue;
				for(var prop in ref_table[i]){
					if(prop == "isRestored")continue;
					if(ref_table[i].hasOwnProperty(prop)){
						if(prop == "$evt_func"){
							code += "\t" + new_disclosure + "." + prop + " = [\n";
							for(var len = 0; len < ref_table[i][prop].length; len++){
								if(ref_table[i][prop][len].isRestored){
									code += "\t" + "($disclosure.get_ref(" + ref_table[i][prop][len].isRestored[0] + "))." + ref_table[i][prop][len].isRestored[1] + ";\n";
									continue;
								}
								var code_seg = ref_table[i][prop].disclosure_toString();
								var matches = code_seg.match(/\$disclosure[0-9]+/g);
								if(matches){
									var passes = code_seg.match(/\$disclosure[0-9]+ = \$disclosure.create/g);
									...
									code_seg = (function patch_code(code_seg, matches){
										...
										if(code_seg.indexOf(index) > 0 && code_seg[code_seg.indexOf(index) - 1] != '.'){
											return code_seg.substr(0, code_seg.indexOf(index)) + "($disclosure.get_ref(" + ref_table[i].$scopes[index] + "))" + patch_code(code_seg.substr(code_seg.indexOf(index) + index.length), matches);
										}
						...
						if(ref_table[i][prop] && ref_table[i][prop].isRestored){
							code += "\t" + new_disclosure + "." + prop + " = " + "($disclosure.get_ref(" + ref_table[i][prop].isRestored[0] + "))." + ref_table[i][prop].isRestored[1] + ";\n";
							continue;
						}
						if(ref_table[i][prop] instanceof Function){
							...
							if(matches){
								code_seg = (function patch_code(code_seg, matches){
									...
									if(code_seg.indexOf(index) > 0 && code_seg[code_seg.indexOf(index) - 1] != '.'){
										return code_seg.substr(0, code_seg.indexOf(index)) + "($disclosure.get_ref(" + ref_table[i].$scopes[index] + "))" + patch_code(code_seg.substr(code_seg.indexOf(index) + index.length), matches);
										//return code_seg.substr(0, code_seg.indexOf(index)) + "$disclosure" + ref_table[i].$scopes[index] + patch_code(code_seg.substr(code_seg.indexOf(index) + index.length), matches);
									}
									return code_seg.substr(0, code_seg.indexOf(index)) + index + patch_code(code_seg.substr(code_seg.indexOf(index) + index.length), matches);
								})(code_seg, matches);
							}
							//}
							code += "\t" + new_disclosure + "." + prop + " = " + code_seg + ";\n";
						}
						else{
							//code += "\t" + new_disclosure + "." + prop + " = " + stringify(ref_table[i][prop], prop) + ";\n";
							code += "\t" + new_disclosure + "." + prop + " = " + iterativeStringify(ref_table[i][prop], prop) + ";\n";
							//code += "\t" + new_disclosure + "." + prop + " = " + stringifyPrototype(ref_table[i][prop], prop) + ";\n";
						}
					}
			...
			for(var global_index = 0; global_index < global_array.length; global_index++){
				...
				if(!window[global_array[global_index]]){
					...
					if(ref_table_index[1] == "$ret_func")
						code += "var " + global_array[global_index] + " = $disclosure.get_ref(" + ref_table_index[0] + ").$ret_func;\n";
					else
						code += "var " + global_array[global_index] + " = $disclosure.get_ref(" + ref_table_index[0] + ")." + ref_table_index[1] + ";\n";
				}
				else{
			...
			*/
			...
			for(var global_index = 0; global_index < global_array.length; global_index++){
				...
				if(global_array[global_index].startsWith("$disclosure_index"))continue;
				...
				if(ref_table_index[0] > -1){
					...
					if(!(window[global_array[global_index]] instanceof Function))code += "var ";
						...
						var func_string = window[global_array[global_index]].disclosure_toString();
			...
			for ( var i = 0; i < eh_index; i++){
				if (event_table[i].type == 0){ // addEventListener
					...
					var ScopeInd = $disclosure.getScopeObj(event_table[i].obj);
					var event = JSON.stringify(event_table[i].event);
					var callback = event_table[i].callback.disclosure_toString();
					if(event_table[i].ref_index){
						if(event_table[i].ref_index instanceof Array){
							for(var ref in event_table[i].ref_index){
								callback = replaceAll(callback, "$disclosure" + event_table[i].ref_index[ref][0], "($disclosure.get_ref(" + event_table[i].ref_index[ref][1] + "))");
							}
						}
						else{
							for(var j in ref_table[event_table[i].ref_index]){
								if(ref_table[event_table[i].ref_index][j] == event_table[i].callback){
									callback = "($disclosure.get_ref(" + event_table[i].ref_index + "))." + j;
								}
							}
						}
					}
					...
					//if(ref_table_index[0] > -1){
					//	if(ref_table_index[1] == "$evt_func")callback = "$disclosure.get_ref(" + ref_table_index[0] + ").$evt_func[" + ref_table_index[2] + "]";
					//	else callback = "$disclosure.get_ref(" + ref_table_index[0] + ")." + ref_table_index[1];
					//}
					if( ScopeInd > -1 ){ // in ref_table
						code += "$disclosure_ref" + ScopeInd + ".disclosure_addEventListener(" + JSON.stringify(event_table[i].ref_index) + ", " + event + ", " + callback + ");\n";
					} else if( event_table[i].obj == window ){ // obj == window
						code += "window.disclosure_addEventListener(" + JSON.stringify(event_table[i].ref_index) + ", " + event + ", " + callback + ");\n";
					} else if ( event_table[i].obj == document ) { // obj == document
						code += "document.disclosure_addEventListener(" + JSON.stringify(event_table[i].ref_index) + ", " + event + ", " + callback + ");\n"; 
					} else { // obj == document's child var obj_str = $disclosure.getRootPath(event_table[i].obj); code += obj_str + ".disclosure_addEventListener(" + event + ", " + callback + ");\n";
						var str = event_table[i].obj.disclosure_toString();
						if(str.includes("[object HTML")){
							str = getHTMLElementPath(event_table[i].obj);
							code += str + ".disclosure_addEventListener(" + JSON.stringify(event_table[i].ref_index) + ", " + event + ", " + callback + ");\n";
						}
					}
				}
					...
					code += "disclosure_setTimeout(" + JSON.stringify(event_table[i].ref_index) + ", " + callback + ", " + time_modify + ");\n";
				}
					...
					//var callback_str = event_table[i].callback.disclosure_toString();
					//var append_str = "$disclosure_ref0.disclosure_setInterval( " + callback_str + ", " + event_table[i].interval + ");"
					//callback_str = callback_str.slice(0,callback_str.lastIndexOf("}")) + append_str + callback_str.slice(callback_str.lastIndexOf("}"));
					//code += "$disclosure_ref0.disclosure_setTimeout( " + callback_str + ", " + remain_time + ");\n";
					var ref_table_index = lookupFunction(event_table[i].callback, "evt");
					var callback = event_table[i].callback.disclosure_toString();
					if(event_table[i].ref_index){
						if(event_table[i].ref_index instanceof Array){
					...
					code += "disclosure_setTimeout(" + JSON.stringify(event_table[i].ref_index) + ", " + callback + ", " + time_modify + ");\n";
					code += "disclosure_setTimeout(" + JSON.stringify(event_table[i].ref_index) + ", " + "disclosure_setInterval(" + JSON.stringify(event_table[i].ref_index) + ", " + callback + ", " + event_table[i].interval + "), " + (time_modify + event_table[i].interval) + ");\n";
				}
			}

			code += "\n";
			code += "// Event attributes re-register \n";
			code += "$disclosure.registerEventAttribs(); \n\n";
		...
		registerEventAttribs : function(){
			for( var i = 0 ; i < event_attrib_table.length ; i++ ) {
				...
				if( funcname in ref_table[0] )
					callback_replace = '$disclosure0.' + callback.trim();
...
Object.prototype.disclosure_getElementById = function(id){
	var ret = this.getElementById(id);
	ret.$method = ".disclosure_getElementById(" + JSON.stringify(id) + ")";
	return ret;
}

Object.defineProperty(Object.prototype, 'disclosure_getElementById', {
	enumerable: false,
	writable: true,
	configurable: true 
});

Object.prototype.disclosure_getElementsByName = function(id){
	var ret = this.getElementsByName(id);
	ret.$obj    = this;
	ret.$method = ".disclosure_getElementsByName(" + JSON.stringify(id) + ")";
	return ret;
}

Object.defineProperty(Object.prototype, 'disclosure_getElementsByName', {
	enumerable: false,
	writable: true,
	configurable: true 
});

Object.prototype.disclosure_getElementsByTagName = function(id){
	var ret = this.getElementsByTagName(id);
	ret.$obj    = this;
	ret.$method = ".disclosure_getElementsByTagName(" + JSON.stringify(id) + ")";
	return ret;
}

Object.defineProperty(Object.prototype, 'disclosure_getElementsByTagName', {
	enumerable: false,
	writable: true,
	configurable: true 
});

Object.prototype.disclosure_getElementsByClassName = function(id){
	var ret = this.getElementsByClassName(id);
	ret.$obj    = this;
	ret.$method = ".disclosure_getElementsByClassName(" + JSON.stringify(id) + ")";
	return ret;
}

Object.defineProperty(Object.prototype, 'disclosure_getElementsByClassName', {
	enumerable: false,
	writable: true,
	configurable: true 
});

Object.prototype.disclosure_addEventListener = function(ref_index, event, callback, evtCapturing){
	$disclosure.addEventHandler(this, event, callback, ref_index, evtCapturing);
	this.addEventListener(event, callback, evtCapturing);
};

Object.defineProperty(Object.prototype, 'disclosure_addEventListener', {
	enumerable: false,
	writable: true,
	configurable: true 
});
...
Object.prototype.disclosure_setTimeout = function(ref_index, callback, interval){
	var startTime = new Date().getTime();
	$disclosure.setTime(this, callback, interval, startTime, ref_index);
	window.setTimeout(callback, interval);          // this->window
};
/*
Object.prototype.disclosure_setTimeout = function(callback, time){
	var startTime = new Date().getTime();
	$disclosure.setTime(this, callback, startTime + time);
...
Object.defineProperty(Object.prototype, 'disclosure_setTimeout', {
	enumerable: false,
	writable: true,
	configurable: true 
});

Object.prototype.disclosure_setInterval = function(ref_index, callback, time){
	var startTime = new Date().getTime();
	$disclosure.setInterval(this, callback, time, startTime, ref_index);
	var endTime = new Date().getTime();
	var delay = endTime - startTime;
	window.setInterval(callback, time);          // this->window
};

Object.defineProperty(Object.prototype, 'disclosure_setInterval', {
	enumerable: false,
	writable: true,
	configurable: true 
});

Object.prototype.disclosure_toString = Object.prototype.toString;

Object.defineProperty(Object.prototype, 'disclosure_toString', {
	enumerable: false,
	writable: true,
	configurable: true 
});

Function.prototype.disclosure_toString = Function.prototype.toString;

Object.defineProperty(Function.prototype, 'disclosure_toString', {
	enumerable: false,
	writable: true,
	configurable: true 
});

var $disclosure0 = $disclosure.create();
